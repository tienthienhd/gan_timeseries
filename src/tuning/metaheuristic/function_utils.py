...
def hho_f5(solution=None, problem_size=None):
    return np.sum(100 * (solution[1:problem_size] - (solution[0:problem_size - 1])) ** 2 + (
                solution[0:problem_size - 1] - 1) ** 2)
...
def hho_f10(solution=None, problem_size=None):
    ...
    return -20 * np.exp(-0.2 * np.sqrt(np.sum(solution ** 2) / problem_size)) - np.exp(
        np.sum(np.cos(2 * np.pi * solution)) / problem_size) + 20 + np.exp(1)
...
def hho_f12(solution=None, problem_size=None):
    return (np.pi / problem_size) * (10 * ((np.sin(np.pi * (1 + (solution[0] + 1) / 4))) ** 2) + np.sum(
        (((solution[1:problem_size - 1] + 1) / 4) ** 2) * (1 + 10 * ((np.sin(np.pi * (1 + (solution[1:problem_size - 1]
                                                                                           + 1) / 4)))) ** 2)) + (
                                                 (solution[problem_size - 1] + 1) / 4) ** 2) + np.sum(
        hho_ufunction(solution, 10, 100, 4))


def hho_f13(solution=None, problem_size=None):
    return 0.1 * (
                (np.sin(3 * np.pi * solution[0])) ** 2 + np.sum((solution[0:problem_size - 2] - 1) ** 2 * (1 + (np.sin(
            3 * np.pi * solution[1:problem_size - 1])) ** 2)) + ((solution[problem_size - 1] - 1) ** 2) * (
                            1 + (np.sin(2 * np.pi *
...
def hho_f15(solution=None, problem_size=None):
    ...
    return np.sum(
        (aK - ((solution[0] * (bK ** 2 + solution[1] * bK)) / (bK ** 2 + solution[2] * bK + solution[3]))) ** 2)
...
def hho_f18(solution=None, problem_size=None):
    return (1 + (solution[0] + solution[1] + 1) ** 2 * (
                19 - 14 * solution[0] + 3 * (solution[0] ** 2) - 14 * solution[1]
                + 6 * solution[0] * solution[1] + 3 * solution[1] ** 2)) * (
                       30 + (2 * solution[0] - 3 * solution[1]) ** 2 * (
                       18 - 32 * solution[0] + 12 * (solution[0] ** 2) + 48 * solution[1] - 36 * solution[0] * solution[
                   1] + 27 *
                       (solution[1] ** 2)))
